package com.example.bookloverfinalapp.app.ui.teacher_screens
...
@HiltViewModel
class FragmentSignUpTeacherViewModel @Inject constructor(
    ...
    private val _classes = MutableSharedFlow<List<Class>>(replay = 1,
        extraBufferCapacity = 0, onBufferOverflow = BufferOverflow.DROP_OLDEST)
    ...
    fun getClasses(classesIds: List<String>) = viewModelScope.launch {
        ...
        classesIds.forEach { id ->
            getClassUseCasCase.execute(id = id).collectLatest { resource ->
                when (resource.status) {
                    Status.LOADING -> showProgressDialog()
                    Status.SUCCESS -> {
                        dismissProgressDialog()
